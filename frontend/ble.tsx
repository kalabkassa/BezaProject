...
var averageHB=0;
var averageTemp=0;
var socket = new WebSocket('ws://192.168.81.203:8000/update/');
      ...
      <View 
       ...
       <View
          ...
          {allDevices.map((device: Device) => (
            <TouchableOpacity  onPress={()=>{try{connectToDevice(device)}catch(e){console.error(e)}}} style={styles.conButton}>
              <Text style={{color: 'white', alignSelf: 'center', fontSize: 12}}>connect {device.name}</Text>
            </TouchableOpacity>
          ))}
      </View>
      </>
